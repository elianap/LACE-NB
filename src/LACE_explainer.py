...
class LACE_explainer:
    ...
    def __init__(self, clf, train_dataset, min_sup=MINIMUM_SUPPORT, dataset_name=None):
        self.clf = clf
        ...
        self.model=clf_names[str(type(self.clf))] if str(type(self.clf)) in clf_names else None


    def explain_instance(self, encoded_instance: pd.Series, decoded_target_class, retObj=True, metas=None):
        """
        ...
        if retObj:
            xp=LACE_explanation(self, single_attribute_differences, deepcopy(difference_map), k, error,
                  self.train_dataset.inverse_transform_instance(encoded_instance),
                  decoded_target_class,
                  errors,
                  target_class_index,
                  self.clf.predict_proba(
                    encoded_instance_x)[0][target_class_index], metas=metas)
        else:
            xp = {'LACE_explainer_o': self, 'diff_single': single_attribute_differences,
